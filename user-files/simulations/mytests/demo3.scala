...
class demo3 extends Simulation {
  ...
  object GetNullHtml {
	...
	object GetRegularHtml {
  	val getRegularHtml = exec(http("getRegularHtml").get("/getHtml?name=${userName}")).pause(1)
	}
	...
	object GetRegularJson {
  	val getRegularJson = exec(http("getRegularJson").get("/getJson?name=${userName}")).pause(1)
	}
