...
class demo5 extends Simulation {
  ...
  object GetNullHtml {
	...
	object GetNullFormJson {
  	val getNullFormJson = exec(http("getNullFormJson").get("/getFormJson").check(status.is(200))).pause(1)
	}

	object GetRegularFormJson {
  	val getRegularFormJson = exec(http("getRegularFormJson").get("/getFormJson?name=${userName}").check(status.is(200))).pause(1)
	}
